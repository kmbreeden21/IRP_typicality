...
function extractCatchCategory(imagePath, plural) {
    ...
    if (plural == true) {
        ...
        return catchSingulars[category];
    }
}

// Function to create catch trial structure
function createCatchTrials() {
    const catchTrials = [];
    
    for (let i = 0; i < catchTrialImages.length; i++) {
        catchTrials.push({
            image: catchTrialImages[i],
            category: extractCatchCategory(catchTrialImages[i], true), // Use plural for question
            isCatchTrial: true
        });
    }
...
// Function to insert catch trials every N trials
function insertCatchTrials(regularTrials, catchTrials, interval = 20) {
    const result = [];
    let catchIndex = 0;
    
    for (let i = 0; i < regularTrials.length; i++) {
        result.push(regularTrials[i]);
        
        // Insert catch trial every 'interval' trials (and if we have catch trials left)
        if ((i + 1) % interval === 0 && catchIndex < catchTrials.length) {
            result.push(catchTrials[catchIndex]);
            catchIndex++;
        }
    }
    
    console.log(`Inserted ${catchIndex} catch trials out of ${catchTrials.length} available`);
    console.log(`Total trials: ${result.length} (${regularTrials.length} regular + ${catchIndex} catch)`);
...
const jsPsych = initJsPsych({
    on_finish: function() {
        ...
        console.log('Number of trials:', jsPsych.data.get()
            .filter({trial_type: 'main_trial'})
            .count());
        console.log('Number of catch trials:', jsPsych.data.get()
            .filter({trial_type: 'catch_trial'})
            .count());
    }
});
...
// Create paired image-category objects for regular trials
var images_paired = [];
...
// Shuffle the regular trials
var images_shuffled = jsPsych.randomization.shuffle(images_paired);

// Create catch trials
var catch_trials = createCatchTrials();

// Insert catch trials every 20 trials
var all_trials = insertCatchTrials(images_shuffled, catch_trials, 20);

// Extract all unique image URLs for preloading
const all_image_urls = [...new Set(all_trials.map(trial => trial.image))];

console.log(`Preloading ${all_image_urls.length} unique images...`);

// Preload trial
const preload = {
    type: jsPsychPreload,
    images: all_image_urls,
    message: '<div style="text-align: center; font-size: 18px; margin-top: 100px;"><p>Loading images...</p><div class="loading-bar-container" style="width: 300px; height: 30px; border: 2px solid #ccc; border-radius: 15px; margin: 20px auto; background-color: #f9f9f9;"><div class="loading-bar" style="height: 100%; background: linear-gradient(90deg, #4CAF50, #45a049); border-radius: 13px; width: 0%; transition: width 0.3s ease;"></div></div><p id="loading-text">0%</p></div>',
    error_message: 'The experiment failed to load. Please refresh the page and try again.',
    show_detailed_errors: true,
    continue_after_error: false,
    on_progress: function(num_loaded, num_total, num_errors) {
        const percent = Math.round((num_loaded / num_total) * 100);
        ...
        console.log(`Preloading progress: ${num_loaded}/${num_total} images loaded (${percent}%)`);
        if (num_errors > 0) {
            console.warn(`${num_errors} images failed to load`);
        }
    },
    on_success: function(data) {
        console.log('✅ All images preloaded successfully!');
        console.log(`Total images loaded: ${data.n_loaded}`);
        console.log(`Load time: ${data.rt}ms`);
    },
    on_error: function(data) {
        console.error('❌ Image preloading failed');
        console.error(`Failed images: ${data.failed_images}`);
...
var trial = {
    ...
    stimulus: function() {
        ...
        let image_name;
        if (isCatch) {
            image_name = extractCatchCategory(imageUrl, false); // Get singular form for display
        } else {
            image_name = extractCategory(imageUrl, false);
        }
        
        console.log('Trial type:', isCatch ? 'CATCH' : 'REGULAR');
...
// Define list of trials using the combined trial list
...
const save_data = {
    ...
    on_finish: function(data) {
        ...
        if (data.success) {
            ...
            // Log catch trial performance
            const catchTrials = jsPsych.data.get().filter({is_catch_trial: true});
            const passedCatch = catchTrials.filter({catch_trial_passed: true}).count();
            console.log(`Catch trial performance: ${passedCatch}/${catchTrials.count()} passed`);
        } else {
...
jsPsych.run([consent, preload, fullscreen_trial, demographics_timeline, instructions, trials_with_variables, comments_trial, save_data, end_fullscreen, completion_code_trial]);